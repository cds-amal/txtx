...
impl RunbookBuilder {
    ...
    pub fn validate_with_mode(&mut self, mode: ValidationMode) -> ValidationResult {
        ...
        match mode {
            ...
            ValidationMode::Doctor { manifest, environment, file_path } => {
                // Use the doctor analyzer for full validation
                use txtx_cli::cli::doctor::analyzer::RunbookAnalyzer;
                
                let analyzer = RunbookAnalyzer::new();
                let path = file_path.unwrap_or_else(|| PathBuf::from("test.tx"));
                
                // Convert CLI inputs from HashMap to Vec of tuples
                let cli_inputs: Vec<(String, String)> = self.cli_inputs
                    .iter()
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();
                ...
                ValidationResult {
                    success: core_result.errors.is_empty(),
                    errors: core_result.errors.into_iter()
                        .map(|e| txtx_addon_kit::types::diagnostics::Diagnostic::error_from_string(e.message))
                        .collect(),
                    warnings: core_result.warnings.into_iter()
                        .map(|w| txtx_addon_kit::types::diagnostics::Diagnostic::warning_from_string(w.message))
                        .collect(),
                }
            }
            ValidationMode::Lsp { workspace_root, manifest } => {
                // TODO: Implement LSP validation mode
                // This would use the LSP's validation pipeline
                unimplemented!("LSP validation mode not yet implemented")
            }
        }
    }
