...
use crate::cli::lsp::multi_file::{load_multi_file_runbook, map_line_to_file, get_runbook_name_for_file};
        ...
        if let Some(runbook_name) = get_runbook_name_for_file(file_uri, manifest) {
            ...
            if let Some(runbook) = manifest.runbooks.iter().find(|r| r.name == runbook_name) {
                ...
                if let Ok(manifest_path) = manifest.uri.to_file_path() {
                    let runbook_path = manifest_path.parent()
...
fn validate_multi_file_runbook(
    ...
    for error in &validation_result.errors {
        if let Some(line) = error.line {
            if let Some((file_path, mapped_line)) = map_line_to_file(line, &multi_file.file_boundaries) {
    ...
    for input_ref in &input_refs {
        ...
        if let Some((file_path, mapped_line)) = map_line_to_file(input_ref.line, &multi_file.file_boundaries) {
            // Only process refs from the current file
            if file_path == current_file_path {
                let input_name = input_ref.name.strip_prefix("input.")
                    .unwrap_or(&input_ref.name);
