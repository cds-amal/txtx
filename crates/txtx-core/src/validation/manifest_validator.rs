...
use super::types::{LocatedInputRef, ValidationResult, ValidationError, ValidationWarning, ValidationSuggestion};
...
pub fn validate_inputs_against_manifest(
    ...
    let rules = if config.strict_mode {
        get_strict_rules()
    } else {
        get_default_rules()
    };
...
fn get_default_rules() -> Vec<Box<dyn ManifestValidationRule>> {
    vec![
        Box::new(UndefinedInputRule),
        Box::new(DeprecatedInputRule),
    ]
}
...
impl ManifestValidationRule for UndefinedInputRule {
    ...
    fn check(&self, context: &ManifestValidationContext) -> ValidationOutcome {
        ...
        if !context.effective_inputs.contains_key(context.input_name) {
            ...
            let cli_provided = context.cli_inputs
                .iter()
                .any(|(k, _)| k == context.input_name);
            ...
            if !cli_provided {
                return ValidationOutcome::Error {
                    ...
                    documentation_link: Some("https://docs.txtx.rs/manifests/environments".to_string()),
                };
            }
        }
...
mod tests {
    ...
    fn test_undefined_input_detection() {
        ...
        let input_refs = vec![
            LocatedInputRef {
                name: "env.undefined_var".to_string(),
                line: 10,
                column: 5,
            },
        ];
    ...
    fn test_cli_input_precedence() {
        ...
        let input_refs = vec![
            LocatedInputRef {
                name: "env.cli_provided".to_string(),
                line: 10,
                column: 5,
            },
        ];
